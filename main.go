...
const logPath = "./linko.access.log"
...
func run(ctx context.Context, cancel context.CancelFunc, httpPort int, dataDir string) int {
	...
	if err != nil {
		stdLogger.Printf("failed to open log file: %v", err)
		return 1
	}
	acsLogger := log.New(f, "INFO: ", log.LstdFlags)
	...
	s := newServer(*st, httpPort, cancel, acsLogger)
	var serverErr error
	go func() {
		serverErr = s.start()
	}()
