...
func (s *server) handlerRedirect(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, store.ErrNotFound) {
			...
			httpError(ctx, w, http.StatusInternalServerError, fmt.Errorf("internal, server error: %v", err))
		}
		return
	}
...
func (s *server) handlerListURLs(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		httpError(ctx, w, http.StatusInternalServerError, fmt.Errorf("failed to list URLs: %v", err))
		return
	}
